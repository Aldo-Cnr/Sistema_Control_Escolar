<?php
    ...
    $stm = $conexion->prepare("SELECT * FROM admin");
    $stm->execute();
    $admins = $stm->fetchAll(PDO::FETCH_ASSOC);


    if(isset($_GET['id_admin'])){
        $txtId = isset($_GET['id_admin'])?$_GET['id_admin']:"";
        $stm = $conexion->prepare('DELETE FROM admin WHERE id_admin = :txtId');
...
<body>
    ...
    <div class="table-responsive mt-4 container-fluid fw-light">
        <table class="table table-primary">
            ...
            <tbody class="table-secondary">
                ...
                <tr class="">
                    <td scope="row"><?php echo $admin['id_admin']; ?></td>
                    <td> <?php echo $admin['nombres'];?></td>
                    <td> <?php echo $admin['apellidos'];?></td>
                    <td><?php echo $admin['edad'];?></td>
                    <td>
                        <a href="./update/editar-admin.php?id_admin=<?php echo $admin['id_admin']; ?>" class="btn btn-outline-success bg-success">Editar</a>
                        <a href="crud-admin.php?id_admin=<?php echo $admin['id_admin']; ?>" class="delete-reg btn btn-outline-danger bg-danger">Eliminar</a>
                    </tr>
                <?Php }?>
    ...
    <?php 
        include('../../views/admin/create/crear-admin.php');
    ?>
