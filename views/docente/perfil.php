<?php
  ...
  $matricula = (isset($_GET['id_docente'])?$_GET['id_docente']:'');
  $stm = $conexion->prepare('SELECT * FROM docentes WHERE id_docente = :matricula');
  $stm->bindParam(":matricula", $matricula);
  $stm->execute();
  $docentes = $stm->fetch(PDO::FETCH_LAZY);
    ...
    <div class="card cont-card text-white d-flex align-items-center justify-content-center">
        ...
        <div class="card-body w-100 h-100 text-center d-flex flex-column justify-content-center align-items-center">
          <div class="row mb-3">
            ...
            <div class="col-md-6">
            <div class="mb-4 mt-1 w-100">
                <label for="status" class="m-2">Profesion:</label>
                <input type="text" class="rounded-3 text-center" value="<?php echo $profesion; ?>" readonly>
              </div>
          ...
          <a href="../../views/docente/editar-perfil.php?id_docente=<?php echo $matricula; ?>" class="btn btn-primary mt-2 mb-5">Editar Perfil</a>
        </div>
      </div>
